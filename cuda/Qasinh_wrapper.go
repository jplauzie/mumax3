package cuda

/*
 THIS FILE IS AUTO-GENERATED BY CUDA2GO.
 EDITING IS FUTILE.
*/

import(
	"unsafe"
	"github.com/mumax/3/cuda/cu"
	"github.com/mumax/3/timer"
	"sync"
)

// CUDA handle for Qasinh kernel
var Qasinh_code cu.Function

// Stores the arguments for Qasinh kernel invocation
type Qasinh_args_t struct{
	 arg_dst unsafe.Pointer
	 arg_src unsafe.Pointer
	 arg_N int
	 argptr [3]unsafe.Pointer
	sync.Mutex
}

// Stores the arguments for Qasinh kernel invocation
var Qasinh_args Qasinh_args_t

func init(){
	// CUDA driver kernel call wants pointers to arguments, set them up once.
	 Qasinh_args.argptr[0] = unsafe.Pointer(&Qasinh_args.arg_dst)
	 Qasinh_args.argptr[1] = unsafe.Pointer(&Qasinh_args.arg_src)
	 Qasinh_args.argptr[2] = unsafe.Pointer(&Qasinh_args.arg_N)
	 }

// Wrapper for Qasinh CUDA kernel, asynchronous.
func k_Qasinh_async ( dst unsafe.Pointer, src unsafe.Pointer, N int,  cfg *config) {
	if Synchronous{ // debug
		Sync()
		timer.Start("Qasinh")
	}

	Qasinh_args.Lock()
	defer Qasinh_args.Unlock()

	if Qasinh_code == 0{
		Qasinh_code = fatbinLoad(Qasinh_map, "Qasinh")
	}

	 Qasinh_args.arg_dst = dst
	 Qasinh_args.arg_src = src
	 Qasinh_args.arg_N = N
	

	args := Qasinh_args.argptr[:]
	cu.LaunchKernel(Qasinh_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream0, args)

	if Synchronous{ // debug
		Sync()
		timer.Stop("Qasinh")
	}
}

// maps compute capability on PTX code for Qasinh kernel.
var Qasinh_map = map[int]string{ 0: "" ,
61: Qasinh_ptx_61  }

// Qasinh PTX code for various compute capabilities.
const(
  Qasinh_ptx_61 = `
.version 8.5
.target sm_61
.address_size 64

	// .globl	Qasinh

.visible .entry Qasinh(
	.param .u64 Qasinh_param_0,
	.param .u64 Qasinh_param_1,
	.param .u32 Qasinh_param_2
)
{
	.reg .pred 	%p<7>;
	.reg .f32 	%f<53>;
	.reg .b32 	%r<20>;
	.reg .b64 	%rd<10>;


	ld.param.u64 	%rd2, [Qasinh_param_0];
	ld.param.u64 	%rd3, [Qasinh_param_1];
	ld.param.u32 	%r3, [Qasinh_param_2];
	mov.u32 	%r4, %ctaid.y;
	mov.u32 	%r5, %nctaid.x;
	mov.u32 	%r6, %ctaid.x;
	mad.lo.s32 	%r7, %r4, %r5, %r6;
	mov.u32 	%r8, %ntid.x;
	mov.u32 	%r9, %tid.x;
	mad.lo.s32 	%r1, %r7, %r8, %r9;
	setp.ge.s32 	%p1, %r1, %r3;
	@%p1 bra 	$L__BB0_8;

	cvta.to.global.u64 	%rd4, %rd3;
	cvt.s64.s32 	%rd1, %r1;
	mul.wide.s32 	%rd5, %r1, 4;
	add.s64 	%rd6, %rd4, %rd5;
	ld.global.nc.f32 	%f1, [%rd6];
	abs.f32 	%f2, %f1;
	setp.gt.f32 	%p2, %f2, 0f7E800000;
	@%p2 bra 	$L__BB0_6;
	bra.uni 	$L__BB0_2;

$L__BB0_6:
	lg2.approx.f32 	%f46, %f2;
	mul.f32 	%f47, %f46, 0f3F317218;
	mov.f32 	%f48, 0f3F317218;
	add.rn.f32 	%f52, %f48, %f47;
	bra.uni 	$L__BB0_7;

$L__BB0_2:
	rcp.rn.f32 	%f10, %f2;
	mov.f32 	%f11, 0f3F800000;
	fma.rn.f32 	%f12, %f10, %f10, %f11;
	sqrt.rn.f32 	%f13, %f12;
	add.f32 	%f14, %f10, %f13;
	rcp.approx.ftz.f32 	%f15, %f14;
	fma.rn.f32 	%f3, %f2, %f15, %f2;
	add.rz.f32 	%f16, %f3, %f11;
	mov.b32 	%r10, %f16;
	add.s32 	%r11, %r10, -1061158912;
	and.b32  	%r12, %r11, -8388608;
	mov.b32 	%r2, %f3;
	sub.s32 	%r13, %r2, %r12;
	mov.b32 	%f17, %r13;
	mov.u32 	%r14, 1082130432;
	sub.s32 	%r15, %r14, %r12;
	mov.b32 	%f18, %r15;
	mov.f32 	%f19, 0fBF800000;
	mov.f32 	%f20, 0f3E800000;
	fma.rn.f32 	%f21, %f20, %f18, %f19;
	add.f32 	%f22, %f21, %f17;
	cvt.rn.f32.s32 	%f23, %r12;
	mul.f32 	%f24, %f23, 0f34000000;
	mov.f32 	%f25, 0f3DD80012;
	mov.f32 	%f26, 0fBD39BF78;
	fma.rn.f32 	%f27, %f26, %f22, %f25;
	mov.f32 	%f28, 0fBE0778E0;
	fma.rn.f32 	%f29, %f27, %f22, %f28;
	mov.f32 	%f30, 0f3E146475;
	fma.rn.f32 	%f31, %f29, %f22, %f30;
	mov.f32 	%f32, 0fBE2A68DD;
	fma.rn.f32 	%f33, %f31, %f22, %f32;
	mov.f32 	%f34, 0f3E4CAF9E;
	fma.rn.f32 	%f35, %f33, %f22, %f34;
	mov.f32 	%f36, 0fBE800042;
	fma.rn.f32 	%f37, %f35, %f22, %f36;
	mov.f32 	%f38, 0f3EAAAAE6;
	fma.rn.f32 	%f39, %f37, %f22, %f38;
	mov.f32 	%f40, 0fBF000000;
	fma.rn.f32 	%f41, %f39, %f22, %f40;
	mul.f32 	%f42, %f22, %f41;
	fma.rn.f32 	%f43, %f42, %f22, %f22;
	mov.f32 	%f44, 0f3F317218;
	fma.rn.f32 	%f52, %f24, %f44, %f43;
	setp.lt.u32 	%p3, %r2, 2139095040;
	@%p3 bra 	$L__BB0_7;

	setp.lt.s32 	%p4, %r2, -1082130431;
	@%p4 bra 	$L__BB0_5;

	mov.f32 	%f45, 0f7F800000;
	fma.rn.f32 	%f52, %f3, %f45, %f45;

$L__BB0_5:
	setp.eq.f32 	%p5, %f3, 0f00000000;
	selp.f32 	%f52, 0f80000000, %f52, %p5;

$L__BB0_7:
	mov.b32 	%r16, %f1;
	and.b32  	%r17, %r16, -2147483648;
	mov.b32 	%r18, %f52;
	or.b32  	%r19, %r17, %r18;
	mov.b32 	%f49, %r19;
	setp.le.f32 	%p6, %f2, 0f7F800000;
	selp.f32 	%f50, %f49, %f52, %p6;
	cvta.to.global.u64 	%rd7, %rd2;
	shl.b64 	%rd8, %rd1, 2;
	add.s64 	%rd9, %rd7, %rd8;
	st.global.f32 	[%rd9], %f50;

$L__BB0_8:
	ret;

}

`
 )
