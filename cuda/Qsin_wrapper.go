...
// CUDA handle for Qsin kernel
var Qsin_code cu.Function

// Stores the arguments for Qsin kernel invocation
type Qsin_args_t struct{
...
// Stores the arguments for Qsin kernel invocation
var Qsin_args Qsin_args_t

func init(){
	// CUDA driver kernel call wants pointers to arguments, set them up once.
	 Qsin_args.argptr[0] = unsafe.Pointer(&Qsin_args.arg_dst)
	 Qsin_args.argptr[1] = unsafe.Pointer(&Qsin_args.arg_src)
	 Qsin_args.argptr[2] = unsafe.Pointer(&Qsin_args.arg_N)
	 }

// Wrapper for Qsin CUDA kernel, asynchronous.
func k_Qsin_async ( dst unsafe.Pointer, src unsafe.Pointer, N int,  cfg *config) {
	...
	Qsin_args.Lock()
	defer Qsin_args.Unlock()

	if Qsin_code == 0{
		Qsin_code = fatbinLoad(Qsin_map, "Qsin")
	}

	 Qsin_args.arg_dst = dst
	 Qsin_args.arg_src = src
	 Qsin_args.arg_N = N
	

	args := Qsin_args.argptr[:]
	cu.LaunchKernel(Qsin_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream0, args)
...
// maps compute capability on PTX code for Qsin kernel.
var Qsin_map = map[int]string{ 0: "" ,
61: Qsin_ptx_61  }

// Qsin PTX code for various compute capabilities.
const(
  Qsin_ptx_61 = `
.version 8.5
.target sm_61
...
.visible .entry Qsin(
	.param .u64 Qsin_param_0,
	.param .u64 Qsin_param_1,
	.param .u32 Qsin_param_2
)
{
	...
	ld.param.u64 	%rd9, [Qsin_param_0];
	ld.param.u64 	%rd10, [Qsin_param_1];
	ld.param.u32 	%r20, [Qsin_param_2];
	add.u64 	%rd1, %SPL, 0;
