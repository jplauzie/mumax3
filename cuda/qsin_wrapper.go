...
// CUDA handle for unary_sin kernel
var unary_sin_code cu.Function

// Stores the arguments for unary_sin kernel invocation
type unary_sin_args_t struct{
...
// Stores the arguments for unary_sin kernel invocation
var unary_sin_args unary_sin_args_t

func init(){
	// CUDA driver kernel call wants pointers to arguments, set them up once.
	 unary_sin_args.argptr[0] = unsafe.Pointer(&unary_sin_args.arg_dst)
	 unary_sin_args.argptr[1] = unsafe.Pointer(&unary_sin_args.arg_a)
	 unary_sin_args.argptr[2] = unsafe.Pointer(&unary_sin_args.arg_N)
	 }

// Wrapper for unary_sin CUDA kernel, asynchronous.
func k_unary_sin_async ( dst unsafe.Pointer, a unsafe.Pointer, N int,  cfg *config) {
	if Synchronous{ // debug
		Sync()
		timer.Start("unary_sin")
	}

	unary_sin_args.Lock()
	defer unary_sin_args.Unlock()

	if unary_sin_code == 0{
		unary_sin_code = fatbinLoad(unary_sin_map, "unary_sin")
	}

	 unary_sin_args.arg_dst = dst
	 unary_sin_args.arg_a = a
	 unary_sin_args.arg_N = N
	

	args := unary_sin_args.argptr[:]
	cu.LaunchKernel(unary_sin_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream0, args)

	if Synchronous{ // debug
		Sync()
		timer.Stop("unary_sin")
	}
}

// maps compute capability on PTX code for unary_sin kernel.
var unary_sin_map = map[int]string{ 0: "" ,
61: unary_sin_ptx_61  }

// unary_sin PTX code for various compute capabilities.
const(
  unary_sin_ptx_61 = `
.version 8.5
.target sm_61
...
.visible .entry unary_sin(
	.param .u64 unary_sin_param_0,
	.param .u64 unary_sin_param_1,
	.param .u32 unary_sin_param_2
)
{
	...
	ld.param.u64 	%rd9, [unary_sin_param_0];
	ld.param.u64 	%rd10, [unary_sin_param_1];
	ld.param.u32 	%r20, [unary_sin_param_2];
	add.u64 	%rd1, %SPL, 0;
