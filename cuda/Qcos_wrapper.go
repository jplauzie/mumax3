...
// CUDA handle for Qcos kernel
var Qcos_code cu.Function

// Stores the arguments for Qcos kernel invocation
type Qcos_args_t struct{
...
// Stores the arguments for Qcos kernel invocation
var Qcos_args Qcos_args_t

func init(){
	// CUDA driver kernel call wants pointers to arguments, set them up once.
	 Qcos_args.argptr[0] = unsafe.Pointer(&Qcos_args.arg_dst)
	 Qcos_args.argptr[1] = unsafe.Pointer(&Qcos_args.arg_src)
	 Qcos_args.argptr[2] = unsafe.Pointer(&Qcos_args.arg_N)
	 }

// Wrapper for Qcos CUDA kernel, asynchronous.
func k_Qcos_async ( dst unsafe.Pointer, src unsafe.Pointer, N int,  cfg *config) {
	...
	Qcos_args.Lock()
	defer Qcos_args.Unlock()

	if Qcos_code == 0{
		Qcos_code = fatbinLoad(Qcos_map, "Qcos")
	}

	 Qcos_args.arg_dst = dst
	 Qcos_args.arg_src = src
	 Qcos_args.arg_N = N
	

	args := Qcos_args.argptr[:]
	cu.LaunchKernel(Qcos_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream0, args)
...
// maps compute capability on PTX code for Qcos kernel.
var Qcos_map = map[int]string{ 0: "" ,
61: Qcos_ptx_61  }

// Qcos PTX code for various compute capabilities.
const(
  Qcos_ptx_61 = `
.version 8.5
.target sm_61
...
.visible .entry Qcos(
	.param .u64 Qcos_param_0,
	.param .u64 Qcos_param_1,
	.param .u32 Qcos_param_2
)
{
	...
	ld.param.u64 	%rd9, [Qcos_param_0];
	ld.param.u64 	%rd10, [Qcos_param_1];
	ld.param.u32 	%r21, [Qcos_param_2];
	add.u64 	%rd1, %SPL, 0;
